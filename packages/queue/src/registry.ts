...
import { createRecipeAiEditQueue } from "./recipe-ai-edit/queue";
...
interface QueueRegistry {
  ...
  recipeAiEdit: Queue<RecipeAiEditJobData>;
...
export async function initializeQueues(): Promise<QueueRegistry> {
  ...
  initializing = (async () => {
    ...
    registry = {
      ...
      recipeAiEdit: createRecipeAiEditQueue(removalOptions),
...
export function getQueueByName(name: QueueName): Queue {
  const byName: Record<QueueName, Queue> = {
    ...
    [QUEUE_NAMES.RECIPE_AI_EDIT]: getQueues().recipeAiEdit,
