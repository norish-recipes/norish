...
- [ ] The store editor adds, renames, reorders and removes aisles, refuses a duplicate name case-insensitively, and writes nothing until Save.
- [ ] Saving a Store with aisles persists them in order; an aisle removed from the list is deleted; a Store deleted takes its aisles with it (repository tests).
- [ ] Every Store event carries the Store with its aisles, and the client merges by store id (tRPC tests).
- [ ] The public stores endpoint lists a Store's aisles; the REST create is unchanged.
- [ ] Every aisle of a Store renders as a slim heading in order in the flat and grouped views, empty ones quieter; all rows stay unfiled at the top; a Store without aisles renders exactly as before.
- [ ] E2E: adding two aisles in the editor makes both headings appear with the list unchanged above them.
