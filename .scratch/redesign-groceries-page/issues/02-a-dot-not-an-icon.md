...
- [ ] No Heroicon is rendered for a Store anywhere on the web; the heading (both views), the selector and the manager rows show the dot; Unsorted and the no-store option show none.
- [ ] The store editor has no icon control; the Aisles list sits directly under the colour picker (component test).
- [ ] One heading component is shared by the flat, grouped and By Recipe sections.
- [ ] A migration drops the column; the Store DTO, the create and update inputs and the client defaults no longer carry an icon.
- [ ] A create that sends an icon is accepted and the created Store has none (tRPC test); the public stores endpoint lists Stores without it.
