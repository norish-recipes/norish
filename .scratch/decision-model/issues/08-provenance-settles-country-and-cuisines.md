...
- 2026-09-20 — Review fix. In gap-fill mode the language model echoes the supplied country and Cuisines and `verifyClaims` judged them, against the rule that validation sees only the claims the run just made: the shadow disagreement rate counted supplied values, and a disputed supplied country would have failed every retry once country validation is promoted. A claim equal to a supplied slot is now skipped (country by normalised code, Cuisine by trimmed lowercase name) and kept whatever the verdict; a country the model chose against the section is still judged. Also `answer.probabilities[answer.choice] ?? 0`: a distribution missing the chosen entry settles nothing, defensively, since the runtime already refuses it. Three tests added.
- 2026-09-24 — The Decision path now runs under `extend` too, at Mike's request. A clear vocabulary Cuisine means something on the list fits, which is exactly when `extend` would not add one; with no Cuisine clear the language model proposes, may name a new one, and each proposal is validated before it is minted. The country Choice never depended on the Cuisine strategy. Test "takes today's single inference under the extend strategy" replaced by two covering both branches.
- 2026-09-24 — Flow reworked at Mike's request, one owner per part. Country: the Choice's top option is taken whatever its probability (`COUNTRY_THRESHOLD` removed), with a `none` option (252 options) that stores no country; the country no longer gates the Cuisines. Cuisines under `existing`: the vocabulary Booleans are final at a plain yes (`CUISINE_THRESHOLD = DROP_THRESHOLD = 0.5`, strictly above), an empty answer included, and the language model is not asked for Cuisines. Cuisines under `extend`: no vocabulary Booleans; the language model proposes and `verifyClaims` runs with a new forced `mode: "enforce"` whenever the Recipe Provenance use is on, so a "no" is neither attached nor minted. A slot settled empty reaches the note's section as `none`. Auto-tagging gained the same shape under a new `autoTagging` Decision Use: `predefined` asks one Boolean per tag parsed from the prompt's `PREDEFINED TAGS:` block (`parsePredefinedTags`), `predefined_db` and `freeform` have the language model propose and the Decision confirm. A Decision failure falls back to the language model everywhere.
