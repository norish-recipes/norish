...
- 2026-09-24 — `DROP_THRESHOLD` raised from 0.2 to 0.5 at Mike's request: everything a run adds must be something the Decision Model agrees with, so a link or tag it answers "no" to is removed even when that "no" is not sure. Allergens keep `ALLERGEN_DROP_THRESHOLD = 0.05`. Boundary test now 0.50 / 0.51.
