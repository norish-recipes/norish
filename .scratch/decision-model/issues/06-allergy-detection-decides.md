...
- 2026-09-20 — Review fix. The Decision path sent every household allergen in one request whatever the count; it now splits at `MAX_QUESTIONS_PER_DECISION` (40) like provenance and validation, through `chunk` exported from `verification.ts`. Tests: 40 allergens is one request, 41 is two (40 + 1). The suite no longer mocks `verifyClaims`; validation runs for real through `decide`, with the 0.05 / 0.06 boundary and the stored-tag test in the language-model describe.
