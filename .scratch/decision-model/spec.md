...
| Allergy detection (language-model path), recipe extraction                            | Same Booleans; a faithfulness Score for extraction                     | Logged only. Allergens are never enforced: dropping an allergen is the one removal that can hurt someone | shadow                                                   | 10     |
