...
- **Admin form.** A _Decision Model_ section joins the AI & Processing accordion after Image Generation: provider select, API key (secret input, masked, revealable, preserved when omitted on save), model field **prefilled** with `jev-latest`, a **Test** button that issues one trivial Boolean question and reports success or the provider's error, one **Use the Decision Model for** multi-select below the connection fields, and the standard unsaved-changes and dirty-section handling. No introductory paragraph, no description under the model field or the multi-select, and **no endpoint control**: the maintainer cut the "advanced" disclosure on 2026-09-21 as something that will never be used. The schema keeps `endpoint` (the E2E harness points the block at its fake provider through the mutation), and the form carries a stored value along on save rather than dropping it. Translations in all fourteen locales; the i18n gate is the check.
...
- **The job monitor names the model.** Every entry point also records the request on a per-job **model-use ledger** (`ai/runtime/model-use-ledger.ts`, AsyncLocalStorage like the operation context): the configured provider key, the model id (for a Decision the resolved id from the response, so the release behind `jev-latest` shows), and whether it completed or failed. The lazy worker manager opens a ledger around every processor and writes what it collected onto the job's current attempt when the processor settles; the admin job detail lists the latest attempt's models once each, failures marked. Added 2026-09-21 at the maintainer's request; no kind returns its wiring, and outside a job the ledger is a no-op.
...
| 11  | Docs, screenshots and release notes, with mermaid diagrams of the internals        | Ships last           | resolved        |
...
Everything in the sequence is implemented on the branch except one box an agent sandbox cannot tick: ticket 04's measurements against a real TypeSafe key (latency, rate-limit headers, the per-request question limit). Ticket 11's screenshot was captured off the E2E production stack on 2026-09-21. Ticket 04 stays `ready-for-human` for the measurements alone.
