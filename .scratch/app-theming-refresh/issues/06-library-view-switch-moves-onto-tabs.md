...
- Follow-up while revising (2026-08-13): the library's first paint flickered — `RecipeGrid`'s `showSkeleton` started `false` and only flipped in an effect, so the grid area painted blank for a frame or two and on a fast network the data popped in with no skeleton at all. It now seeds from `isLoading`, so the skeleton is part of the first (and server) paint and the load is a clean skeleton-to-data swap; verified frame-by-frame.
