...
- [ ] 2.1 Convert `ratings.rate`; establish the conversion recipe, update output typing and tests.
...
- [ ] 2.3 Convert `stores.delete`.
- [ ] 2.4 Convert `groceries.update`, `groceries.reorderInStore`, `groceries.markAllDone`, `groceries.deleteDone` (+ output schemas).
- [ ] 2.5 Convert `groceries.updateRecurring`, `groceries.detachRecurring`, `groceries.deleteRecurring`, `groceries.checkRecurring`; remove `flushAsync` hacks from recurring tests.
- [ ] 2.6 Convert `recipes.create` (keeps string return), `recipes.update`, `recipes.delete`, `recipes.convertMeasurements`.
- [ ] 2.7 Convert `households.create/join/leave/kick/regenerateCode/transferAdmin` with awaited cache invalidation and deferred connection termination.
- [ ] 2.8 Retire `failed` event emission from converted mutations; verify hooks single-report errors.
- [ ] 2.9 Formalize `archive.importArchive` enqueue contract (`{ success: true, status: "accepted", total }`).

## 3. Client reconciliation

- [ ] 3.1 Add `stale`-flag reconciliation (`if (result.stale) invalidate()`) to shared-react mutation hooks where missing.
- [ ] 3.2 Add hook tests proving optimistic render appears immediately and rolls forward/back on applied, stale, and thrown-error outcomes.

## 4. Verification

- [ ] 4.1 Update the audit matrix and the shrink-only test list as each router converts.
