...
describe("useGroceriesMutations", () => {
  ...
  describe("optimistic grocery merge (shared rule, ADR-0009)", () => {
    it("merges into a matching cached grocery instead of inserting a duplicate", async () => {
      const existing = createMockGrocery({
        id: "existing",
        name: "Apples",
        unit: "piece",
        amount: 2,
      ...
      queryClient.setQueryData(["groceries", "list"], createMockGroceriesData([existing], []));
      ...
      act(() => {
        result.current.createGrocery("1 piece Apples");
      });
      ...
      expect(data?.groceries).toHaveLength(1);
      expect(data?.groceries[0]).toMatchObject({
        id: "existing",
        amount: 3,
        version: 4,
    ...
    it("does not merge into unit-incompatible or done rows", async () => {
      const incompatible = createMockGrocery({
        id: "kg-row",
        name: "Apples",
        unit: "kg",
        amount: 2,
      });
      const done = createMockGrocery({
        id: "done-row",
        name: "Apples",
        unit: "piece",
        isDone: true,
      ...
      queryClient.setQueryData(
        ["groceries", "list"],
        createMockGroceriesData([incompatible, done], [])
      );
      ...
      expect(data?.groceries).toHaveLength(3);
      expect(data?.groceries[0]).toMatchObject({ name: "Apples", unit: "piece", isDone: false });
      expect(data?.groceries[0]?.id).not.toBe("kg-row");
      expect(data?.groceries[0]?.id).not.toBe("done-row");
    });

    it("merges identical batch items into one optimistic row (many-to-one)", async () => {
      queryClient.setQueryData(["groceries", "list"], createMockGroceriesData([], []));
      ...
      act(() => {
        void result.current.createGroceriesFromData([
          { name: "Salt", amount: 1, unit: null },
          { name: "salt ", amount: 3, unit: null },
        ]);
      });
      ...
      expect(data?.groceries).toHaveLength(1);
      expect(data?.groceries[0]).toMatchObject({ name: "Salt", amount: 4 });
    });

    it("keeps recipe-sourced and manual items apart in a batch", async () => {
      queryClient.setQueryData(["groceries", "list"], createMockGroceriesData([], []));
      ...
      act(() => {
        void result.current.createGroceriesFromData([
          { name: "Salt", amount: 1, unit: null, recipeIngredientId: "ri-1" },
          { name: "Salt", amount: 1, unit: null },
        ]);
      });
      ...
      expect(data?.groceries).toHaveLength(2);
    });
  });
