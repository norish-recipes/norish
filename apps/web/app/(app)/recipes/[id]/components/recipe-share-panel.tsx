...
export default function RecipeSharePanel({ open, onOpenChange }: Props) {
  ...
  const { invalidateRecipeShares, invalidateMyRecipeShares, invalidateAdminRecipeShares, invalidateRecipeShare } =
  ...
  const shareRows = useMemo(() => shares.slice().sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime()), [shares]);
    ...
    <Panel height={PANEL_HEIGHT_LARGE} open={open} title={t("title")} onOpenChange={onOpenChange}>
      <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
                <Select
                  ...
                  onSelectionChange={(keys) => {
                    const selected = Array.from(keys)[0] as CreateRecipeShareInputDto["expiresIn"] | undefined;
          ...
          <div className="flex min-h-0 flex-1 flex-col gap-3">
            <h3 className="text-sm font-semibold uppercase tracking-wide">{t("currentLinks")}</h3>
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="text-default-500 mt-2 space-y-1 text-xs">
                            <p>{t("createdAt", { value: formatDate(share.createdAt) ?? t("never") })}</p>
                            <p>{t("expiresAt", { value: formatDate(share.expiresAt) ?? t("never") })}</p>
                            {share.lastAccessedAt && (
                              <p>{t("lastAccessedAt", { value: formatDate(share.lastAccessedAt) })}</p>
                            )}
                          </div>
                        </div>
