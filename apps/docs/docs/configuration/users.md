...
The bin button asks for confirmation and then removes the account for good. **NO ARCHIVING OF ACCOUNTS**
...
- **Their recipes stay on the server.** A recipe outlives the account that
  added it and becomes unowned (orphaned). An unowned (orphaned) recipe is visible to everyone on the
